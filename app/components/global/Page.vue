<template>
    <div v-if="$slots.toolbar" class="shrink-0 flex items-center justify-between border-b border-default px-4 sm:px-6 gap-1.5 overflow-x-auto min-h-[50px]">
        <slot name="toolbar" />
    </div>
    <div class="flex flex-col gap-4 sm:gap-6 p-4 sm:p-6 overflow-y-auto">
        <div v-if="title || description || $slots.actions" class="relative flex flex-col gap-5 lg:flex-row lg:items-center">
            <div :class="['flex-1 space-y-1', alignCenter ? 'text-center' : undefined]">
                <div class="text-2xl text-pretty font-semibold text-highlighted">
                    {{ title }}
                </div>
